"use client";
import { useCreateQuizAssessment } from "@/features/assessments/hooks/useAssessment";
...
function QuizModal({
  ...
  const { mutateAsync } = useCreateQuizAssessment();
  const quizChangeHanlder = (type:string) => {
...
const updateAnswer = (e: React.ChangeEvent<HTMLInputElement>, quizId: string, quizTitle: string, selected: string) => {
  const checked = e.target.checked;

  // If the radio/checkbox is unchecked → remove the answer for this quiz
  if (!checked) {
    setAnswers(prev => prev.filter(a => a.quizId !== quizId));
    return;
  }

  // User selected an option → create the new answer object
  const newAnswer = {
    quizId,
    options: [{ option: selected }],   // adjust the shape to what your backend expects
  };

  setAnswers(prev => {
    // Remove old answer for this quiz (if any) and add the new one
