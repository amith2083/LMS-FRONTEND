...
import { useAddQuizToQuizset } from "@/features/quizzes/hooks/useQuizQueries";
...
export const AddQuizForm = ({ quizSetId }) => {
  ...
  //     toast.error("You must mark only one option as correct");
  //     return;
  //   }

  //   // Construct options array
  //   const options = [
  //     { label: values.optionA.label, isTrue: values.optionA.isTrue },
  //     { label: values.optionB.label, isTrue: values.optionB.isTrue },
  //     { label: values.optionC.label, isTrue: values.optionC.isTrue },
  //     { label: values.optionD.label, isTrue: values.optionD.isTrue },
  //   ];
...
const onSubmit = async (values) => {
  ...
  try {
    ...
    console.log("Correctness flags:", correctness);

    const correctMarked = correctness.filter((c) => c);
    console.log("Correct options marked:", correctMarked.length);

    const isOneCorrectMarked = correctMarked.length === 1;

    if (!isOneCorrectMarked) {
      console.warn("Validation failed: More than one or none marked correct.");
      toast.error("You must mark only one option as correct");
      return;
    }
